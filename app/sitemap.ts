...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    return [
      ...urls,
      ...tracks.docs.map((item) => ({ url: `${base}/learning-paths/${item.id}`, lastModified: new Date(), changeFrequency: "weekly" as const, priority: 0.8 })),
      ...modules.docs.map((item) => ({ url: `${base}/room/${item.id}`, lastModified: new Date(), changeFrequency: "weekly" as const, priority: 0.7 })),
